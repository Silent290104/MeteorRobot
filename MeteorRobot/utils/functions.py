...
from MeteorRobot import aiohttpsession as aiosession
from MeteorRobot.ex_plugins.dbfunctions import start_restart_stage
from MeteorRobot.utils.http import get, post


async def restart(m: Message):
    if m:
        await start_restart_stage(m.chat.id, m.message_id)
    execvp(executable, [executable, "-m", "MeteorRobot"])
...
def generate_captcha():
    ...
    height = 100
    correct_answer = ""
    font = ImageFont.truetype("MeteorRobot/resources/default.ttf", 55)
    file = f"MeteorRobot/resources/{randint(1000, 9999)}.jpg"
