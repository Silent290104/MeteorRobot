...
from MeteorRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./MeteorRobot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Meteor Robot](https://t.me/Silent_Robo_Bot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./MeteorRobot/resources/fonts"):
            os.remove(font_)
